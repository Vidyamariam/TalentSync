...
import { Problem } from "../../src/data/problems";

interface ProblemsDescriptionProps {
    ...
    <div className="h-full overflow-y-auto bg-base-200">
      ...
      <div className="p-6 space-y-6 ">
        <div className="bg-base-100 rounded-xl shadow-sm p-5 border border-base-300">
          <h1 className="text-lg font-bold text-base-content">Description</h1>

          <div className="space-y-4 bg-base-100">
            <p className="text-base-content leading-relaxed">
              {currentProblem?.description.text}
            </p>
            {currentProblem?.description?.notes.map((note, i) => (
              <p key={i}>{note}</p>
            ))}
          </div>
        </div>
